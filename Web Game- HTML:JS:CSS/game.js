...
Player.prototype.draw = function () {
    ...
    ctxEntities.drawImage(imgSprite, this.srcX, this.srcY,
                          this.width, this.height, this.drawX,
                          this.drawY, this.width, this.height);
};
...
Player.prototype.checkShooting = function () {
    if (this.isSpacebar && !this.isShooting) {
        ...
        this.isShooting = false;
        document.getElementById("canvasBg").innerHTML = "Score is : " ;
    }
};
...
Bullet.prototype.fire = function (startX, startY) {
    var soundEffect = new Audio("shooting.wav");
    soundEffect.play();
    this.drawX = startX;
    this.drawY = startY;

    if (PlayerOne.srcX === 0) { // Facing south
        this.xVel = 0;
        this.yVel = this.speed;
    }
    else if (PlayerOne.srcX === 35) { // Facing north
        this.xVel = 0;
        this.yVel = -this.speed;
    }
    else if (PlayerOne.srcX === 70) { // Facing west
        this.xVel = -this.speed;
        this.yVel = 0;
    }
    else if (PlayerOne.srcX === 105) { // Facing east
        this.xVel = this.speed;
        this.yVel = 0;
    }
    this.isFlying = true;
...
Enemy.prototype.die = function () {
    var soundEffect = new Audio("dying.wav");
    soundEffect.play();
...
function checkKey(e, value) {
    ...
    if (keyID === 38) { // Up arrow
        PlayerOne.isUpKey = value;
        e.preventDefault();
    }
    if (keyID === 39) { // Right arrow
        PlayerOne.isRightKey = value;
        e.preventDefault();
    }
    if (keyID === 40) { // Down arrow
        PlayerOne.isDownKey = value;
        e.preventDefault();
    }
    if (keyID === 37) { // Left arrow
        PlayerOne.isLeftKey = value;
        e.preventDefault();
    }
    if (keyID === 32) { // Spacebar
        PlayerOne.isSpacebar = value;
        e.preventDefault();
    }
}
